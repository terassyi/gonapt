...
if [ "$BUILD" = "$1" ]; then
	...
	sudo ip netns exec napt ip link add br0 type bridge
	...
	sudo ip link set np-l netns napt
	sudo ip link set np-w netns napt
	sudo ip link set br0-v0 netns napt
	sudo ip link set br0-v1 netns napt
	sudo ip link set br0-v2 netns napt
	sudo ip link set br0-v3 netns napt
	sudo ip link set srv-v0 netns srv

	echo "setting bridge"
	sudo ip netns exec napt ip link set dev br0-v0 master br0
	sudo ip netns exec napt ip link set dev br0-v1 master br0
	sudo ip netns exec napt ip link set dev br0-v2 master br0
	sudo ip netns exec napt ip link set dev br0-v3 master br0
	...
	sudo ip netns exec napt ip addr add 10.0.0.254/24 dev np-l
	sudo ip netns exec napt ip addr add 138.76.28.4/24 dev np-w
	sudo ip addr add 138.76.28.1/24 dev r-0
	sudo ip addr add 138.76.29.254/24 dev r-1
	...
	sudo ip netns exec napt ip link set up np-l
	sudo ip netns exec napt ip link set up np-w
	sudo ip netns exec napt ip link set up br0
	sudo ip netns exec napt ip link set up br0-v0
	sudo ip netns exec napt ip link set up br0-v1
	sudo ip netns exec napt ip link set up br0-v2
	sudo ip netns exec napt ip link set up br0-v3
	sudo ip netns exec napt ip link set up lo
	sudo ip netns exec srv ip link set up srv-v0
	sudo ip netns exec srv ip link set up lo
	sudo ip link set up r-0
	sudo ip link set up r-1
	...
	sudo ip netns exec napt ip route add default via 138.76.28.1
	sudo ip netns exec srv ip route add default via 138.76.29.254


elif [ "$CLEAN" = "$1" ]; then
	sudo ip netns del srv
	sudo ip netns del napt
